				...
				<div id="branch" class="branch">
					...
					<div class="branch entertain_box" id="project">
						...
						<?php
							...
							$row = $res->fetch_array();
							$num = $row["c"];
							$query = "SELECT news_id, title, description, image FROM news WHERE news_id ORDER BY news_id DESC LIMIT 5 OFFSET ". ($id*5-5) .";";
							$res = $db->query($query);
							if($res->num_rows > 0){
						...
						<div class="row ">
							<div class="col-md-4 mar">
								<div class="team-member">
									<div class="image"><a href='php/article.php?id=<?php echo $row["news_id"]; ?>'><img src='<?php echo $row["image"]; ?>' alt="" class="img-responsive"></a></div>
									
								</div>
							</div>
							<div class="col-md-8  mar">
								<div class="team-member">
									<h3 class="head"><a href='php/article.php?id=<?php echo $row["news_id"]; ?>'> <?php echo $row["title"]; ?> </a></h3>
									<p> <?php echo $string ?>
										</p><a href='php/article.php?id=<?php echo $row["news_id"]; ?>' class="read" style="text-decoration:none;">READ MORE</a>
									</div>
								</div>
						...
						<?php for ($i=1; $i <= ceil($num/5) ; $i++) { 
							if($i == $id) {
								echo "<a href='index.php?page=".$i."' style='margin-right:10px;'><u><b>".$i."</b></u></a>";
							}
							else {
								echo "<a href='index.php?page=".$i."' style='margin-right:10px;'><u>".$i."</u></a>";	
							}
						} ?>
						</center>
					</div>
