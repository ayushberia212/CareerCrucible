...
<?php
	...
	$id = $_GET["id"];
	$query = "SELECT title, description, image, link, b_img FROM news where news_id=".$id.";";
	$res = $db->query($query);
	if($res->num_rows == 1){
	$row = $res->fetch_array();
	$string = strip_tags($row["description"]);
		...
		$string = substr($string, 0, 500);
			...
			$remaining = substr($row["description"], strlen($string), strlen($row["description"]));
		}
?>
<html>
	<head>
				...
				<div id="branch" class="branch">
					...
					<div class="branch entertain_box" id="project">
						...
						<div class="row" style="margin-top:20px;padding-left:40px;">
							...
							<p>FOR DETAILS ABOUT EXAMINATION visit  <a href='<?php echo $row["link"]; ?>'><strong>Here</strong></a></p>
							<?php } ?>
						</div>
